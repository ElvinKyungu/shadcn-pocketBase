      ...
      <div class="relative p-4 w-full max-w-2xl max-h-full z-50">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
          <div v-if="isSubmit" class="h-96 flex justify-center items-center">
            <div class="p-4 md:p-5 text-center" v-if="errorMessage !==''">
              ...
              <span class="text-red-700 text-lg">{{ errorMessage}}</span>
            </div>
            <Spinner v-else/>
          </div>
          <div v-else>
  ...
  if (validateFormTask(newTask.value)){
    ...
    if(userStore.userID !== null){
      try{
        ...
        await addNewTaskTEST(newTask.value)
        setTimeout(()=>{
          closeModal()
        }, 2000)
      }catch{
        errorMessage.value = "Impossible d'ajouter une tâche"
